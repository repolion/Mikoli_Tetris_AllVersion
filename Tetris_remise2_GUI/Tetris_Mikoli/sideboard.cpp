...
void SideBoard::setDisplay(){
    ...
    if(!this->_game->isGameOver() && !this->_game->isPaused()){
        for(Block b: this->_game->getNextFig().getBlocks()){
            ...
            SideBoard::paint(b,block_width,b.getColor(),2,2,5,5,1,true);
        }
    }

    //Affichage timer
    // std::pair<int, int> temp = _game->getTimer()->statutTimeGame();
    ...
    QString playerWinQ = "Player wins";
    QString nextQ = "Next:";
    ...
    //Affiche "pause"
    if(this->_game->isBegin() && this->_game->isPaused() ){
        SideBoard::displayLabel(pausedQ,15,9,20,Qt::red,20);
    }

    //Affiche "Player won" ou "Game Over"
    if(this->_game->isGameOver() && this->_game->isWon()){
        SideBoard::displayLabel(playerWinQ,10,5,30,Qt::darkBlue,40);
    }
    else if(this->_game->isGameOver()){
        SideBoard::displayLabel(gameOverQ,10,4,30,Qt::red,40);
    }
