...
ChoiceBoard::ChoiceBoard(QWidget & fenetre, TetrisGame * game){
    ...
    //Affecte la palette au QRadioButtons
    ...
    Buttons butRestart = Buttons(fenetre,526,475,100,50,QString("Restart"));
    Buttons butNewGame = Buttons(fenetre,526,357,150,50,QString("New Game"));
    Buttons butQuickGame = Buttons(fenetre,526,300,150,50,QString("Quick Game"));
    Buttons butSingle = Buttons(fenetre,95,300,170,50,QString("1 Player"));
    Buttons butTwoPlayers = Buttons(fenetre,95,365,170,50,QString("2 Players"));
    Buttons butQuit = Buttons(fenetre,526,415,100,50,QString("Quit"));
    Buttons butHome = Buttons(fenetre,526,415,100,50,QString("Home"));
    Buttons butJoin = Buttons(fenetre,526,300,150,50,QString("Join Game"));

    Buttons butServerOn = Buttons(fenetre,true);
    Buttons butServerOff = Buttons(fenetre,false);
    ...
    _buttonTwoPlayers = butTwoPlayers.getButton();
    _buttonQuit = butQuit.getButton();
    _buttonSinglePlayer = butSingle.getButton();
    _buttonHome = butHome.getButton();
    _buttonJoin = butJoin.getButton();

    _buttonServerOn = butServerOn.getButton();
    _buttonServerOff = butServerOff.getButton();
    ...
    butQuit.setVisibility(false);
    butRestart.setVisibility(false);
    butHome.setVisibility(false);
    butServerOn.setVisibility(false);
    butJoin.setVisibility(false);
    hide();

    //GodMode

    _buttonGodMode = new QPushButton("", &fenetre);
    _buttonGodMode->setGeometry(QRect(QPoint(622, 573), QSize(40, 47)));
    _buttonGodMode->setFocusPolicy(Qt::NoFocus);
    _buttonGodMode->setStyleSheet(
    ...
    QObject::connect(_buttonTwoPlayers,SIGNAL(clicked()),&fenetre,SLOT(twoPlayers()));
    QObject::connect(_buttonSinglePlayer,SIGNAL(clicked()),&fenetre,SLOT(singlePlayer()));
    QObject::connect(_buttonQuit,SIGNAL(clicked()),&fenetre,SLOT(quit()));
    QObject::connect(_buttonHome,SIGNAL(clicked()),&fenetre,SLOT(home()));
    QObject::connect(_buttonServerOn,SIGNAL(clicked()),&fenetre,SLOT(serverOn()));
    QObject::connect(_buttonServerOff,SIGNAL(clicked()),&fenetre,SLOT(serverOff()));
    QObject::connect(_buttonJoin,SIGNAL(clicked()),&fenetre,SLOT(quickGame()));
    QObject::connect(_buttonGodMode,SIGNAL(clicked()),&fenetre,SLOT(godMode()));

    _ipDialog = new IpDialog(fenetre);

  //  QObject::connect(_buttonTwoPlayers, SIGNAL(clicked()), _ipDialog, SLOT(showIpDialog()));
...
void ChoiceBoard::hide(){
    ...
    // _buttonTwoPlayers->setVisible(false);
}
...
QPushButton *ChoiceBoard::getButtonTwoPlayers(){
    return _buttonTwoPlayers;
}

QPushButton *ChoiceBoard::getButtonSinglePlayer(){
    return _buttonSinglePlayer;
}

QPushButton *ChoiceBoard::getButtonQuit(){
    return _buttonQuit;
}
...
QPushButton *ChoiceBoard::getButtonHome(){
    return _buttonHome;
}

QPushButton *ChoiceBoard::getButtonServerOn(){
    return _buttonServerOn;
}

QPushButton *ChoiceBoard::getButtonServerOff(){
    return _buttonServerOff;
}

QPushButton *ChoiceBoard::getButtonJoin(){
    return _buttonJoin;
}

QPushButton *ChoiceBoard::getButtonGodMode(){
    return _buttonGodMode;
}
...
void ChoiceBoard::Update(){
    ...
    if(!_game->isSinglePlayer() && _game->isGameOver()){
        _buttonRestart->setVisible(true);
    }
